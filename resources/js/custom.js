...
$(document).ready(function() {
    ...
    $(document).on('click', '.info_icon', function(e) {
        $metaData = $(this).parent().find('.resource_modal_info').html();
        $coverType = $(this).parent().attr('class').split(' ')[1];
        $coverLevel = $(this).parent().find('.resource-meta-data.level').attr('id');
        $coverUrl = $(this).parent().find('.resource-meta-data.url').attr('id');
        $coverStyle = 'cover-' + $coverType + 'l' + $coverLevel;
        
        $displayCover = '<a href="' + $coverUrl + '" target="_blank"><div class="resource-cover ' + $coverStyle + '"></div></a>';
        
        $('.modalContainer').append($displayCover + $metaData).parent().fadeIn(300);
        e.preventDefault();
    });
    ...
    $(document).on('change','.select-lesson select', function() {
        ...
        if( $lesson == 'choose' ) {
            ...
            $('.' + $lu + 'frame .lesson#' + $lesson).parent().parent().parent().parent().removeClass('hiddenlesson');
        }
    });
